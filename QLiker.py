...
def MsgHandler():
    ...
    for i in range (0,len(fkey)):
        try:
            ...
            if abstime is None:
                continue
            logging.info('current QQ num:'+btn_string.group(1).split('/')[3])
            if (btn_string.group(1).split('/')[3] not in specQQnum):
                continue
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            errtime = 0
            reload_config_count += 1
            if(reload_config_count == reload_config_interval):
                with open("config.txt", "r") as cf:
                    for line in cf:
                        specQQnum.add(line.strip())
                logging.info(str(specQQnum))
                reload_config_count = 0
